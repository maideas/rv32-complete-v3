...
## Q1: Most important RISC-V extensions for performance

> Which RISC-V extensions are the most important regarding performance for a hardware implementation that has separate instruction and data buses without caches, 32-bit data width, and some instruction memory wait states?
...
## Q2: Parallel execution path for compressed instructions

> Does it make sense to implement, parallel to the usual execution path, a second one that can execute a compressed instruction in the same cycle?
...
## Q3: Serial execution in debug mode

> When implementing such parallel execution, does it make sense to switch to fully serial execution in debug mode?
