 ...
 * M-mode-only reference model (see riscv_zicsr.hpp), so every generated
 * instruction targets an existing CSR. Read-only CSRs are only paired
 * with non-writing forms (rs1 = x0 / uimm = 0) so that the generated
 * stimulus is trap-free on a correct implementation.
 ******************************************************************************/
...
public:
    ...
    uint32_t writable_csr() {
        ...
        return pool[range(0, (sizeof(pool) / sizeof(pool[0])) - 1)];
    }
